<template>
  <div id="container">
    ...
    <div style="height: 120px;text-align: left;background-color: #f5f5f5;margin-bottom: 10px">
      <el-row>
    ...
    <el-row>
      <el-col :span="10" :offset="4">
        <div style="margin-bottom:5px;font-size:13px;color:gray">{{district}}</div>
      </el-col>
    </el-row>
    <el-row>
      <el-col :span="10" :offset="4">
      ...
      <el-col :span="6" :offset="1" style="width: 300px">
        <el-form label-position="left" inline class="demo-table-expand">
          ...
          <el-form-item label="面积" class="info-special">
            <span class="areaSize">{{info.areaSize}}</span>
            <sub>
              <span class="unit">平 米</span>
            </sub>
          </el-form-item>
          <br />
    ...
    <el-row>
      <el-col :span="16" :offset="4">
        <el-divider content-position="center">
          <span style="font-weight: bold;font-size: 20px">详细信息</span>
        </el-divider>
        <el-col :span="8" :offset="4">
          <ul class="helpInfo">
            <li>面积: {{info.areaSize}}平方米</li>
            <li>供暖: {{info.heating}}</li>
            <li>用电: {{info.electric}}</li>
            <li>用水: {{info.water}}</li>

            <li>楼层: {{info.level}}楼</li>
          </ul>
        </el-col>
        <el-col :span="8" :offset="4">
          <ul class="helpInfo">
            <li>朝向: {{info.orientedDirection}}</li>

            <li>燃气: {{info.naturalGas==0?'无':'有'}}</li>

            <li>车位: {{info.parkingLot==0?'无':'有'}}</li>

            <li>装修: {{info.renovation}}</li>
          </ul>
        </el-col>
      </el-col>
    </el-row>
...
export default {
      ...
      if (this.$store.state.globalCollections.indexOf(this.info.id) > -1) {
        this.$confirm("确认取消收藏?", "取消收藏", {
          ...
          .then(() => {
            this.$axios
              .get(
                "/collection/deleteOne.do?houseId=" +
                  this.info.id +
                  "&userId=" +
                  this.$store.state.globalUserId
              )
              .then(res => {
        ...
        this.$axios
          .post("/collection/addOne.do", form)
          .then(res => {
            if (res.data > 0) {
  ...
  mounted() {
    this.$axios
      .get("/house/getOne.do?id=" + this.$route.params.id)
      .then(res => {
        let house = res.data;
        console.log(res);
        house.pubDate = getDateStr(house.pubDate);
        if (house.length == "err") {
          this.$message.info("此房源不存在!");
        } else {
          this.info = house;
          ...
          // 获取本套房子的收藏数
          this.$axios
            .get("/collection/getCount.do?houseId=" + house.id)
            .then(res => {
          ...
          // 请求房屋主人的信息
          this.$axios
            .get("/user/getOne.do?ownerId=" + house.ownerId)
            .then(res => {
          ...
          // 请求房屋的图片
          this.$axios
            .get("/image/url.do?houseNumber=" + house.houseNumber)
            .then(res => {
