<template>
    ...
    <div>
        ...
        <el-dialog title="更新房源信息" :visible.sync="updateHouseInfoDialog"
                   @close="handleDialogClose"
                   :close-on-click-modal="false" :center="true" :destroy-on-close="true" width="70%">
                        ...
                        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px"
                            ...
                            <el-form-item label="所在地区" prop="areaId">
                                <el-row>
                                    <el-col :span="8">
                                        <div>
                                            <el-select v-model="ruleForm.provinceId" placeholder="请选择" size="mini"
                                                       @change="getCity">
                                                <el-option
                                    ...
                                    <el-col :span="8">
                                        <div>
                                            <el-select v-model="ruleForm.cityId" placeholder="请选择" size="mini"
                                                       @change="getArea">
                                                <el-option
                            ...
                            <el-form-item label="详细地址" prop="address">
                                <el-input type="text" v-model="ruleForm.address" placeholder="所在街道和门牌号"
                                          @focus="forceLogin"></el-input>
                            </el-form-item>
                            <el-form-item label="房屋面积" prop="areaSize">
                                <el-input type="text" v-model="ruleForm.areaSize" placeholder="单位:平方"
                                          @focus="forceLogin"></el-input>
                            </el-form-item>
                            ...
                            <el-form-item label="房屋图片" prop="">
                                <input type="file" multiple style="display:none" id="fileElem" @change="getFile"
                                       @focus="forceLogin"/>
        ...
        <el-row>
            <el-divider><span class="title"> 我 发 布 的 房 源 ({{tableData.length}}) </span></el-divider>
        </el-row>
        <el-row>
            <el-table
                    :data="tableData"
        ...
        methods: {
            ...
            handleDelete(index, row) {
                ...
                this.$confirm('此操作不可恢复，确认删除?', '删除已发布房源', {
                    ...
                    this.$axios.get("/house/deleteOne.do?houseNumber="+houseNumber).then(res=>{
                        if (res.data > 0){
            ...
            getCity() {
                this.$axios("/district/location.do?pId=" + this.ruleForm.provinceId).then(res => {
            ...
            getArea() {
                this.$axios("/district/location.do?pId=" + this.ruleForm.cityId).then(res => {
                    this.areas = res.data;
                }).catch(err => {
                    console.log(err);
                });
            ...
            getImages() {
                // 请求房屋的图片
                this.$axios.get("/image/url.do?houseNumber=" + this.ruleForm.houseNumber).then(res => {
                    if (res.data == "err") {
                        this.fileList = []
                    } else {
                        this.fileList = res.data;
                        for (let i = 0; i < this.fileList.length; i++) {
                            let str = "<img width='150px' height='150px' style='border-radius:2px' src='" + this.fileList[i] + "'/>";
            ...
            deleteImg(index) {
                ...
                if (typeof (img) == "string") {
                    ...
                    this.$axios.delete("/image/deleteOne.do", param).then(res => {
                        console.log(res)
                    })
                }
                    ...
                    if (valid) {
                        ...
                        this.$axios.post("/upload/img.do", files).then(res => {
                            if (res.data == "ok") {
                                ...
                                this.$axios.post("/house/update.do", form).then(res => {
                                    if (res.data > 0) {
        ...
        mounted() {
            ...
            this.$axios.get("/house/published.do").then(res => {
                ...
                for (let i = 0; i < res.data.length; i++) {
                    ...
                    this.$axios.get("/district/singleLocation.do?provinceId=" + data.provinceId +
                        "&cityId=" + data.cityId +
                        "&areaId=" + data.areaId).then(res => {
